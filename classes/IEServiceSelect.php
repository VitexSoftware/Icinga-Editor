...
class IEServiceSelect extends EaseHtmlSelect
{
    ...
    {
        $MembersFound = array('' => '---');
        $query = 'SELECT  `service_id`, `icon_image`,`platform`,`service_description` FROM `' . DB_PREFIX . 'services` WHERE (user_id=' . $this->user->getUserID() . ' OR public=1) AND register=1 ORDER BY  service_description ';
