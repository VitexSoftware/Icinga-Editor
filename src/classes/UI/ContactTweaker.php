    ...
    public function finalize()
    {
        ...
        foreach ($subcontatcts as $subcontatctID => $subcontatctInfo) {
            $this->addItem(
              ...
              new \Ease\Html\ATag('contact.php?parent_id=' . $this->contact->getId() . '&contact_id=' . $subcontatctID, \Ease\TWB\Part::GlyphIcon('wrench') . ' ' . _('Vlastnosti')),
              new \Ease\Html\ATag('?contact_id=' . $this->contact->getId() . '&delsubcont_id=' . $subcontatctID, \Ease\TWB\Part::GlyphIcon('minus') . ' ' . _('smazat') . ' ' . $subcontatctInfo['type'])
                )
                )
            );
        ...
        if (count($this->subcontactTypes)) {
            ...
            $form->addItem(new \Ease\Html\Select('contact', $this->subcontactTypes));
            $form->addItem(new \Ease\Html\InputHiddenTag('contact_id', $this->contact->getId()));
            $form->addItem(
                new \Ease\TWB\FormGroup(_('Kontakt'), new \Ease\Html\InputTextTag('cnt', $this->cnt), \Ease\Shared::webPage()->getRequestValue('cnt'), _('telefonní číslo, email či jabberová adresa dle druhu kontaktu')
                )
            );
            $form->addItem(new \Ease\TWB\SubmitButton(_('Uložit'), 'success'));

            $this->addItem(new \Ease\TWB\Panel(_('Přidat kontaktní údaj'), 'default', $form));
        } else {
            $this->addItem(new \Ease\Html\DivTag('plno', _('K tomuto kontaktu již není možné přidávat další údaje.'), array('class' => 'well warning', 'style' => 'margin: 10px')));
        }
    }
