 ...
 */
class IconSelector extends EaseContainer
{
    ...
    public function __construct($host)
    {
            ...
            if (is_object($d)) {
                while (false !== ($entry = $d->read())) {
                    if (!is_dir($customIcos . '/' . $entry)) {
                        ...
                        $icons->addItem(new \Ease\Html\ATag('?action=newicon&host_id=' . $host->getId() . '&newicon=' . $subicodir . '/' . $entry, $hostIcon));
                    }
                }
                $d->close();
            }
        ...
        $uplBox = $this->addItem(new \Ease\TWB\Panel(_('Nahraj vlastní'), 'info'));
        $uplBox->setTagCss('width: 100%;');
        $icoupform = $uplBox->addItem(new \Ease\TWB\Form('icoUp', null, 'POST', null, array('enctype' => 'multipart/form-data')));
        $icoupform->addItem(new \Ease\Html\EmTag(_('Obrázek typu GIF,PNG nebo JPG, nejlépe 40x40 px')));
        $icoupform->addItem(new \Ease\TWB\FormGroup(_('Ikona ze souboru'), new \Ease\Html\InputFileTag('icofile'), '', _('Soubor v počítači')));
        $icoupform->addItem(new \Ease\TWB\FormGroup(_('Ikona z adresy'), new \Ease\Html\InputTextTag('icourl'), '', _('Soubor na internetu')));
        $icoupform->addItem(new \Ease\TWB\FormGroup(_('Titulek obrázku'), new \Ease\Html\InputTextTag('icon_image_alt'), '', _('nepovinné')));
        $icoupform->addItem(new \Ease\TWB\SubmitButton(_('Odeslat')));
        $icoupform->addItem(new \Ease\Html\InputHiddenTag('host_id', $host->getId()));
    }
