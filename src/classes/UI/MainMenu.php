...
class MainMenu extends \Ease\Html\Div
{
    ...
    private function changesButton($nav)
    {
        if (\Ease\Shared::user()->getSettingValue('unsaved') == true) {
            ...
            $nav->addMenuItem(new \Ease\Html\ATag('apply.php', _('Uplatnit změny'), array('class' => 'btn btn-inverse')), 'right');
        }
    }
    ...
    private function groupsHostsMenu($nav)
    {
        ...
        \Ease\Shared::webPage()->addJavaScript("$('.dropdown-menu').css('max-height',$(window).height()-100);", null, true);
        ...
        if ($pocHostgroup) {
            ...
            foreach ($hostgroups as $cID => $hgInfo) {
                $hostGroupMenuItem['hostgroup.php?hostgroup_id=' . $hgInfo['hostgroup_id']] = \Ease\TWB\Part::GlyphIcon('cloud') . ' ' . $hgInfo['hostgroup_name'];
...
//            $nav->addDropDownMenu(_('Hosti'), $hostGroupHostsMenuItem);
            if (\Ease\Shared::user()->getSettingValue('admin')) {
                ...
                if (count($services)) {
                    $services_menu = array('services.php' => \Ease\TWB\Part::GlyphIcon('list') . ' ' . _('Přehled služeb'));
            ...
            if (count($contacts)) {
                $contacts_menu = array('contacts.php' => \Ease\TWB\Part::GlyphIcon('list') . ' ' . _('Přehled Kontaktů'));
