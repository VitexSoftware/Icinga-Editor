...
class UsedServiceSelector extends \Ease\Container {
    ...
    public function __construct($host) {
        ...
        if (is_null($host->getMyKey())) {
            ...
            $servicesAssigned = $service->listingQuery()->where('host_name LIKE :hn', [':hn' => '%' . $host->getName() . '%'])->orderBy($service->nameColumn)->fetchAll();
                    ...
                    if (isset($request['addservice'])) {
                        ...
                        if ($service->saveToSQL()) {
                            ...
                            if ($service->getDataValue('autocfg') == '1') {
                                ...
                                \Ease\WebPage::singleton()->redirect('servicetweak.php?host_id=' . $request ['host_id'] . '&service_id=' . $request[$service->keyColumn]);
                                exit();
                            }
                        } else {
