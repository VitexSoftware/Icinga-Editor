...
switch ($oPage->getRequestValue('action')) {
    ...
    case 'delete':
        ...
        $confirmator->addItem(new \Ease\TWB\LinkButton('?' . $host->keyColumn . '=' . $host->getID(),
                        _('No') . ' ' . \Ease\TWB\Part::glyphIcon('ok'), 'success'));
        $confirmator->addItem(new \Ease\TWB\LinkButton('?delete=true&' . $host->keyColumn . '=' . $host->getID(),
    ...
    default :
        ...
        $form = $hostParams->addItem(new \Ease\Html\Form(['name' => 'Host', 'action' => 'host.php', 'class' => 'form-horizontal'], $hostEdit));
...
');
        ...
        $renameForm = new \Ease\TWB\Form(['name' => 'Rename', 'action' => '?action=rename&amp;host_id=' . $host->getID()]);
        ...
        if (!empty($host->getDataValue('parents'))) {
