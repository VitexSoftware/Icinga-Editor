...
if ($users) {
    ...
    foreach ($users as $cId => $cInfo) {
        ...
        $lastRow = $cntList->addRowColumns([$cid++, $userInRow,
          ...
          new \Ease\Html\ATag('apply.php?force_user_id=' . $cId, _('Regenerate configuration') . ' <i class="icon-repeat"></i>'),
...
$oPage->columnIII->addItem(new \Ease\TWB\LinkButton('createaccount.php', _('Create user') . ' ' . \Ease\TWB\Part::GlyphIcon('edit')));
