...
$form = $oPage->columnII->addItem(new \Ease\Html\Form(['name'=>'Period','action' => 'timeperiod.php','class' => 'form-horizontal'], $timepriodEdit));
...
foreach ($timeperiod->timeperiods as  $timeName =>  $timeIntervals) {
    $timesTable->addRowColumns([$timeName, $timeIntervals, new \Ease\Html\ATag('?del = '.$timeName.'&amp;
        '.$timeperiod->getKeyColumn().' = '.$timeperiod->getMyKey(),
